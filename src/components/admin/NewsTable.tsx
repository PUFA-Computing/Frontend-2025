...
export default function NewsTable({ news }: { news: News[] }) {
    return (
        <ul role="list" className="divide-y divide-gray-100">
            {news.map((news) => (
                <li key={news.id} className="flex items-center justify-between gap-x-6 py-5">
                    <div className="min-w-0">
                        <div className="flex items-start gap-x-3">
                            <p className="text-sm font-semibold leading-6 text-gray-900">{news.title}</p>
                            <p
                        ...
                        <div className="mt-1 flex items-center gap-x-2 text-xs leading-5 text-gray-500">
                            <p className="whitespace-nowrap">
                                Published on {new Date(news.publish_date).toDateString()}
                            </p>
                            <svg viewBox="0 0 2 2" className="h-0.5 w-0.5 fill-current">
                                <circle cx={1} cy={1} r={1} />
                            </svg>
                            <p className="truncate">Created by {news.author}</p>
                        </div>
                    </div>
                    <div className="flex flex-none items-center gap-x-4">
                        ...
                        >
                            View News<span className="sr-only">, {news.title}</span>
                        </a>
                                ...
                                <MenuItems className="absolute right-0 z-10 mt-2 w-32 origin-top-right rounded-md bg-white py-2 shadow-lg ring-1 ring-gray-900/5 focus:outline-none">
                                    <MenuItem>
                                        {({ focus }) => (
                                            ...
                                            >
                                                Edit<span className="sr-only">, {news.id}</span>
                                            </a>
                                        )}
                                    </MenuItem>
                                    <MenuItem>
                                        {({ focus }) => (
                                            ...
                                            >
                                                Move<span className="sr-only">, {news.id}</span>
                                            </a>
                                        )}
                                    </MenuItem>
                                    <MenuItem>
                                        {({ focus }) => (
                                            ...
                                            >
                                                Delete<span className="sr-only">, {news.id}</span>
                                            </a>
                                        )}
                                    </MenuItem>
                                </MenuItems>
